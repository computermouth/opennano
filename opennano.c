...
enum KEY_ACTION {
    BACKSPACE = 8, TAB = 9,
    CTRL_K = 11, ENTER = 13, CTRL_N = 14, CTRL_O = 15, CTRL_P = 16, CTRL_R = 16,
    CTRL_U = 21, CTRL_W = 23, CTRL_X = 24, CTRL_Y = 25, ESC = 27, CTRL_QUESTIONM = 31, FORWARD_DELETE = 127,
...
#define SEARCH_AND_RESTORE_SYNTAX_HIGHLIGHT_MODE do { \
    ...
    while (true) {
        ...
        if (key == DEL_KEY || key == BACKSPACE || key == FORWARD_DELETE) {
            ...
            free(saved_hl);
            return;
        } else if (key == ARROW_RIGHT || key == ARROW_DOWN || key == CTRL_W) {
            search_next = 1;
...
static void editor_process_keypress(void) {
    ...
    if (key == ENTER) {
        ...
        if (E.dirty && quit_confirmations_left--) {
            editor_set_status_message("WARNING! Unsaved changes. Press ^X one more time to confirm.");
            return;
        } else {
    ...
    } else if (key == CTRL_QUESTIONM) {
        editor_set_status_message("Commands: ^? = Help | ^O = Save | ^X = Quit | ^W = Search | ^K = Cut | ^U = UnCut");
    } else {
        if (key >= 0 && key <= 31) {
